...
export class NonvegComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.activaterouted.paramMap.subscribe( (param) =>
    {
          ...
          console.log(typeof(param.get('name')));
            this.getnonvegbyname(param.get('name'))
          }
    });
  ...
  { 
    ...
    this.dish.getnonveg().subscribe(
      (data) =>
        {
          this.nonveges = data;
          console.log("inside")
          console.log(this.nonveges)
  ...
  getnonvegbyname(name:any)
  {
    { 
      this.dish.getnonvegbyname(name).subscribe(
        (data) =>
          {
            this.nonveges = data;
            console.log("inside")
  ...
  search()
  {
    let strURLForProductsFilterById = "nonveg";

    if (this.name != "")
      strURLForProductsFilterById = "nonveg/"+this.name; // product1/185
